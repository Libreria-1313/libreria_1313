 const libros = [
    ...
    const divbooks = document.getElementById('divbooks'); // Selecciona el div que contendrá los libros
    const libroSelect = document.getElementById('libro-select'); // Selecciona el elemento select para los libros
    
    // Cargar libros cuando se carga la página
    loadBooks();
    
    function listBooks() {
        divbooks.innerHTML = ''; // Limpiar contenido anterior del div de libros
    ...
    function createBook(book) {
        ...
        increaseStockButton.addEventListener('click', () => increaseStock(book)); // Incrementar stock
    
        const decreaseStockButton = document.createElement('button');
        decreaseStockButton.textContent = '-';
        decreaseStockButton.addEventListener('click', () => decreaseStock(book)); // Decrementar stock
        ...
        myBook.appendChild(stockContainer); // Añadir el contenedor del stock al libro
    
        divbooks.appendChild(myBook); // Añadir el libro al contenedor principal
    }
    
    // Función para buscar y eliminar un libro
    function searchAndDeleteBook() {
        const searchCriteria = document.getElementById('searchCriteria').value; // Obtener criterio de búsqueda
        const searchValue = document.getElementById('searchValue').value.toLowerCase(); // Obtener valor de búsqueda
        ...
        for (let i = 0; i < libros.length; i++) {
            ...
            if (
                ...
                libros.splice(i, 1); // Eliminar libro del arreglo
                bookFound = true;
                break;
            }
        }
    ...
    function addBookOption(book) {
        ...
        libroSelect.appendChild(option); // Añadir opción al selector
    }
    
    // Evento que se dispara cuando se selecciona un libro en el select
    libroSelect.addEventListener('change', (event) => {
        const selectedBookTitle = event.target.value;
        showBookDetails(selectedBookTitle); // Mostrar detalles del libro seleccionado
    });
    
    function showBookDetails(selectedBookTitle) {
        const selectedBook = libros.find(book => book.titulo === selectedBookTitle);
    
        // Mostrar detalles del libro seleccionado en algún elemento del DOM
    ...
    function cambiarEditorial() {
        const nuevaEditorial = document.getElementById('nueva-editorial-input').value; // Obtener nueva editorial
        const selectedBookTitle = libroSelect.value; // Obtener título del libro seleccionado
    
        if (selectedBookTitle) {
            const selectedBook = libros.find(book => book.titulo === selectedBookTitle);
            selectedBook.editorial = nuevaEditorial; // Cambiar editorial
    ...
    function agregarNuevoLibro() {
        ...
        nuevoLibro.fecha_publicacion = prompt('Ingrese la fecha de publicación del libro:');
        ...
        nuevoLibro.stock = parseInt(prompt('Ingrese el stock del libro:'));
        ...
        if (
            ...
            nuevoLibro.idioma && !isNaN(nuevoLibro.precio) && nuevoLibro.formato &&
            !isNaN(nuevoLibro.isbn) && nuevoLibro.descripcion && nuevoLibro.estado &&
            nuevoLibro.ubicacion && nuevoLibro.fecha_publicacion && nuevoLibro.editorial &&
            !isNaN(nuevoLibro.paginas) && !isNaN(nuevoLibro.stock) && nuevoLibro.Image
        ) {
            libros.push(nuevoLibro); // Agregar el nuevo libro al array
    ...
    // Función para guardar la lista de libros en el almacenamiento local
    function saveBooksToLocalStorage() {
        localStorage.setItem('libros', JSON.stringify(libros)); // Guardar libros en formato JSON
        alert('Lista de libros guardada en almacenamiento local');
    }
    
    // Función para cargar la lista de libros desde el almacenamiento local
    function loadBooksFromLocalStorage() {
        const storedBooks = localStorage.getItem('libros');
        if (storedBooks) {
            libros = JSON.parse(storedBooks); // Parsear libros desde JSON
            listBooks(); // Mostrar lista de libros cargados
        } else {
            alert('No hay libros guardados en el almacenamiento local');
        }
    }
